document.addEventListener('DOMContentLoaded', () => {
    ...
    if (collapseBtn) {
        collapseBtn.addEventListener('click', () => {
            const isCollapsed = overlayTitle.style.display === 'none';
            overlayTitle.style.display = isCollapsed ? 'block' : 'none';
            overlayDesc.style.display = isCollapsed ? 'block' : 'none';
            collapseBtn.querySelector('.material-icons-round').textContent = isCollapsed ? 'expand_more' : 'expand_less';
        });
    }
    ...
    if (templateBtn) {
        templateBtn.addEventListener('click', function () {
            window.open('https://drive.google.com/drive/folders/1NC2Q2W9HxicnZOm2fgmFIUC9JLKRiYUL?usp=drive_link', '_blank');
        });
    }

    if (photoInput && uploadedPhoto) {
